...
class UserVisitManager(models.Manager):
    ...
    def build(self, request: HttpRequest, timestamp: datetime.datetime) -> UserVisit:
        ...
        uv = UserVisit(
            user_acc=request.user if request.user.is_authenticated else None,
            timestamp=timestamp,
...
class UserVisit(models.Model):
    ...
    user_acc = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="user_visits",blank=True, null=True, default=None, on_delete=models.CASCADE
    )
    timestamp = models.DateTimeField(
        help_text=_lazy("The time at which the first visit of the day was recorded"),
        default=timezone.now,
    )
    session_key = models.CharField(help_text="Django session identifier", max_length=40, blank=True, null=True, default="")
    ...
    @property
    def user(self):
        return self.user_acc if self.user_acc else "Anonymus Visitor"
    ...
    def md5(self) -> hashlib._Hash:
        ...
        h = hashlib.md5(self.remote_addr.encode())
        h.update(self.date.isoformat().encode())
        h.update(self.ua_string.encode())
        if self.user_acc:
            h.update(str(self.user_acc.id).encode())  # noqa: S303, S324
